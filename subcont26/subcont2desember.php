...
?>
    <br>
        ...
        <?php $ambil = $koneksi->query("SELECT * FROM subcont2_desember") ?>
        <?php while ($performancereportdec = $ambil->fetch_assoc()) { ?>
        ...
        <tbody>
            <tr>
                <td><?php echo $performancereportdec['id'] ?></td>
                <td><?php echo $performancereportdec['nama_file'] ?></td>
                <td><a href="../subdev_reports/<?php echo $performancereportdec['nama_file'];?>" class="btn btn-success"><i class="fa fa-download"></i></a></td>
            </tr>
        </tbody>
        </table>
        <?php } ?>
