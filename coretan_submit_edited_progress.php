...
<div class="card col-lg-9 mx-auto">
  ...
  <br>
    ...
    <label for="po-product-item-no" class="col-sm-4 col-form-label">Product Item No.</label>
      ...
      <button type="submit" name="submit-edit" id="submit-edit" class="btn btn-success mx-auto">Submit Perubahan</button>  
      </div>
      <br>
  </div>
  </form>
...
include 'koneksi.php';
  ...
  if(isset($_POST['submit-edit'])){
    ...
    $result = mysqli_query($koneksi,"UPDATE delivery SET Statuss = '$status', additional_notes='$additionalnotes' WHERE id_delivery='$id'");
...
<thead class="thead bg-success">
    ...
    <tbody>
        <?php
        $no = 1;
        $koneksi = mysqli_connect("localhost", "root", "patriaute", "ssmanagement");
        ...
        $query = mysqli_query($koneksi,"SELECT * FROM subcont5_delivery WHERE Po_No like '%".$keyword."%' OR Nama_Subcont like '%".$keyword."%' OR PN like '%".$keyword."%' OR Product_Item_No like '%".$keyword."%' OR Statuss like '%".$keyword."%' ORDER BY id_delivery DESC");
